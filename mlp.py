...
def mlp_objective(model, data, labels, loss_function):
    ...
    #print("squash_derivatives", len(squash_derivatives))
    ...
    for i in reversed(range(num_layers - 1)):
        ...
        layer_errors[i] = np.multiply(np.dot(weights[i+1].transpose(), layer_errors[i+1]), squash_derivatives[i])
    ...
    for i in range(num_layers):
        ...
        #None # replace this with your code
        #print("i", i)
        #print("activation[i].shape", activations[i].shape)
        #print("layer_errors[i].shape", layer_errors[i].shape)
        layer_gradients[i] = np.dot(layer_errors[i], activations[i].transpose())
        #print("layer_gradients.shape", layer_gradients[i].shape)
...
def mlp_train(data, labels, params, model=None):
    ...
    if not model:
        ...
        #print("model weights len", np.random.randn(num_hidden_units[0], input_dim + 1).shape)
        ...
        #print("model_weights", model['weights'])
        model['squash_function'] = params['squash_function']
        #print("squash_function", model['squash_function'])
    ...
    for t in range(params['max_iter']):
        ...
        for j in range(num_layers):
            #print("grad_shape", grad[j].shape)
            #print("model['weights'][j]", model['weights'][j].shape)
